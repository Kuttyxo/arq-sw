...
def llamar_servicio(nombre_servicio, accion, params):
    ...
    try:
        ...
        if match:
            ...
            return {"ok": False, "error": f"Respuesta invalida del servicio '{nombre_servicio}'"}
...
def reservar(datos):
    ...
    for r in reservas:
        if r.get("rut_alumno") == rut and r.get("id_clase") == id_clase and r.get("estado") == "activa":
    ...
    if not resp_plan.get("vigente"):
        return {"ok": False, "error": "El plan del alumno esta vencido o inactivo"}
    ...
    print(f"  → Reserva creada: {nueva_reserva['id_reserva']}")
